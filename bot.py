...
if not BOT_TOKEN:
    print("Укажите токен бота в переменной окружения TELEGRAM_BOT_TOKEN")
    exit(1)

if not config.OPENROUTER_API_KEY:
    print("Укажите OpenRouter API ключ в переменных окружения OPENROUTER_KEY_1 / _2 / _3")
    exit(1)
...
def cmd_help(message):
    send(message.chat.id, parse_mode="HTML", text = 
         "<b>✨ ПОМОЩЬ ✨\n\n</b>"
...
def _start_interrogation(chat_id: int, uid: int, char_id: str):
    ...
    if not history:
        # Первый визит — показываем вступительную реплику и сохраняем в историю
